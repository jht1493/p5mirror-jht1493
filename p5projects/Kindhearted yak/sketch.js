...
function setup() {
  ...
  for (let i = 0; i < 10; i++) {
    let x = random(0, xwidth);
    let y = random(0, yheight);
    let r = random(10, 50);
    tennisballs[i] = new tennisBall(x, y, r, 1, 0);
  }
}
...
function keyPressed() {
  let r = random(10, 50);
  let b = new tennisBall(mouseX, mouseY, r, 2, 3);
  tennisballs.push(b);
}
...
class tennisBall {
  constructor(x, y, r, sx, sy) {
    ...
    this.move = function () {
      ...
      if (this.y > yheight - this.r || this.y < this.r) {
        this.speedy *= -1;
      }
    };

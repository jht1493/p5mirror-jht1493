// https://editor.p5js.org/jht1493/sketches/w5Crcu-U9
...
function draw() {
  drawCircle(width / 2, 280, maxlevel);
}
...
function drawCircle(x, radius, level) {
  ...
  const tt = 255 * level/maxlevel;
  fill(tt);
  ellipse(x, height / 2, radius * 2, radius * 2);
  if (level > 1) {  
    // 'level' decreases by 1 at every step 
    // and thus makes the terminating condition attainable
    level = level - 1;  

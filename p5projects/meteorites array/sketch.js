// https://editor.p5js.org/jht1493/sketches/E8VTO0IaL
// https://editor.p5js.org/Lisha_Nie/sketches/XxYGh8grl
// meteorites
let arr = []; // array of objects { vid, playVideo, file, fileM }
...
function setup() {
  ...
  for (let index = 0; index < 9; index++) {
    let ent = {};
    let index1 = index + 1;
    ent.file = index1 + ".mp4"; // eg. "1.mp4"
    ent.fileM = "M" + index1 + ".mp4"; // eg. "M1.mp4"
    ent.vid = createVideo(ent.file);
    ent.vid.position(x, y);
    ent.vid.size(250, 250);
    ent.vid.loop();
    ent.x = x;
    ent.y = y;
    ent.playVideo = false;
...
function draw() {
  ...
  for (let index = 0; index < 9; index++) {
    let ent = arr[index];
    if (ent.playVideo) {
      console.log("playVideo", ent);
      ent.vidM = createVideo(ent.fileM);
      ent.vidM.position(ent.x, ent.y);
      ent.vidM.size(250, 250);
      ent.vidM.loop();
      ent.playVideo = false;
    }
  }
...
function mouseClicked() {
  ...
  for (let index = 0; index < 9; index++) {
    ...
    if (
      ...
      console.log("mouseClicked ent", ent);
      if (ent.vidM) {
        ent.vidM.remove();
        ent.vidM = false;
      } else {

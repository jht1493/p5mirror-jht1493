// first phase of debugging 17.1_playMode ui 
let a_song;
let a_volume = 0.5;
let a_pan = 0;
let a_rate = 1;
...
function setup() {
  ...
  // currentTime stuck at zero
  // a_song = loadSound(song_file+'.mp3', sound_loaded);
  
  // currentTime ok, sometimes
  // a_song = loadSound('media/Reference Scales_Chromatic Scale On F Sharp.mp3',sound_loaded);
  loadSound('media/Reference Scales_Chromatic Scale On F Sharp.mp3',sound_loaded);
  
  sketch_ui({ playModes, song_list });
}

function sound_loaded(asong) {
  a_song = asong;
